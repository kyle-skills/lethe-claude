#!/usr/bin/env python3
"""
smart-compact: Session Discovery + Environment Detection

Finds a Claude Code session by watermark UUID and detects the terminal
environment for orchestrated relaunch.

Modes:
    Discovery:      compact-discover.py <WATERMARK_UUID> [--pid <PID>]
    Terminal only:  compact-discover.py --detect-terminal <PID>
...
Exit codes:
    0 = success
    1 = bad arguments
    2 = not found (watermark or terminal)
"""
...
# {cwd} and {command} are substituted at runtime
TERMINAL_TEMPLATES = {
...
def extract_session_metadata(jsonl_path: Path) -> dict:
    """Extract cwd, version, gitBranch from the JSONL's last few entries.

    Reads from the end since metadata fields are on most entries and
    the latest values are most relevant.
    """
    ...
    while current_pid and current_pid > 1 and current_pid not in visited:
        ...
        if comm:
            ...
            for alias, canonical in TERMINAL_ALIASES.items():
                if comm.startswith(alias) or comm == alias:
...
def main():
    parser = argparse.ArgumentParser(
        description="smart-compact: Session discovery + environment detection"
    )
    parser.add_argument(
        "watermark",
        nargs="?",
    ...
    if args.pid is not None:
        ...
        if terminal_name:
            ...
            result["terminal_launch"] = TERMINAL_TEMPLATES[terminal_name].format(
                cwd=shlex.quote(cwd), command="{command}"
            )
